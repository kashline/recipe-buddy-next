...
export const GET = async function (request: NextRequest) {
    ...
    if (user === undefined){
        return Response.json({success: false, message: `No valid user session!`}, {status: 403})
    }
    ...
    if (user === undefined || user.sub !== userId){
        return Response.json({success: false, message: `User sub ID doesn't match provided user ID!`})
    }
