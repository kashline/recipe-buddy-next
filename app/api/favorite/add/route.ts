...
export const POST = async (request: NextRequest) => {
    try {
        ...
        if (data.recipeId !== undefined || data.recipeId !== null){
            ...
            return Response.json(({success: true, message: `Successfully added recipe ${data.recipeId} to user ${userId}'s favorites.`}), {status: 200})
        }
    } catch (error) {
        console.log(`There was an error adding a favorite recipe: ${error}`)
        return Response.json(({success: false, message: `There was an error adding a favorite recipe: ${error}`}), {status: 500})
    }
}
